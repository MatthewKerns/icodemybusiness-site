# Free Offers for the Community

Internal reference for all free resources and lead magnets we provide.

---

## 1. Free Discovery Calls (5/month)

- **Format:** 30-minute video call via Calendly
- **Value:** $100 per call (normally $200/hr)
- **Availability:** 5 slots per month (scarcity-driven)
- **Booking link:** https://calendly.com/matthewkerns/discovery-free
- **What they get:**
  - Deep dive into their specific business or wellness goals
  - Custom AI system recommendation
  - Action plan they can implement on their own
- **Goal:** Convert to paid consulting or managed execution

---

## 2. Free Organic Marketing Audit

- **Format:** Video call via Calendly
- **Booking link:** https://calendly.com/matthewkerns/organic-audit
- **What's included:**
  - Analysis of current CAC and ad spend efficiency
  - Review of organic presence gaps
  - Infinity Vault case study walkthrough
  - Custom recommendations for their brand
  - No pitch unless they ask
- **Goal:** Convert to Tier 1-3 service packages

---

## 3. AI Agent Blueprints (Blog Content)

- **Format:** Long-form blog posts — real Claude Code plans that became working systems
- **Location:** `/blog/business/` and `/blog/wellness/`
- **Current posts:**
  - Business: Data Contract & Integration Analysis
  - Wellness: Building an AI Wellness Coach
- **What they get:** Full technical blueprint they can study or reference
- **Goal:** Demonstrate expertise, drive discovery call bookings

---

## 4. Wellness Financial Health Blueprint (Bonus)

- **Format:** PDF/document delivered after booking a wellness discovery call
- **Value:** $500 (positioned)
- **Availability:** Limited time bonus
- **Goal:** Increase wellness discovery call conversion rate

---

## 5. Claude Series: Project Management with Claude (Content Series)

- **Format:** Recurring blog posts + short social clips (LinkedIn, TikTok)
- **Concept:** One continuous project management example that builds across episodes
- **What it covers:** Using Claude Projects, API automation, MCP, Agent Skills — all through managing a real client project
- **Cadence:** Batch 5-10 posts, publish daily
- **Each episode ties to an Anthropic Academy course** for deeper learning
- **Full plan:** `/planning/claude-series-project-management.md`
- **Goal:** Demonstrate expertise, drive discovery calls, build community trust

---

## 6. About Matthew Story

- **Format:** Long-form page at `/blog/about-matthew.html`
- **What it covers:** Ex-Amazon background, burnout-to-recovery journey, company vision
- **Goal:** Build trust and relatability, drive discovery call bookings

---

## Pricing Ladder After Free Offers

| Stage | Price | Notes |
|-------|-------|-------|
| Free discovery call | $0 | 5 slots/month |
| First paid call | $50/hr | 75% off for new clients |
| Ongoing consulting | $100/hr | 50% off standard rate |
| Standard rate | $200/hr | Full price |

---

*Last updated: March 2026*
