...
| 2026-09-05T13:20Z | 7216322 | Merge branch 'agent/cmo/launch-readiness' | lint/tsc/test green on VPS | staging-verified (script checks) | matthewkerns@Drs-MacBook-Pro |
| 2026-09-06T20:48Z | da66437 | docs(team): board — goal met and observed: /admin/funnel renders the constraint identifier behind the owner login | lint/tsc/test green on VPS | staging-verified (script checks) | matthewkerns@Drs-MacBook-Pro |
| 2026-09-06T22:00Z | 9d15b03 | docs(log): the shared VPS hit 0 bytes free mid-gate-check (includes 926d293 homepage-crash fix + Convex push) | lint/tsc/test green on VPS | staging-verified (script checks) — image built cleanly (build.log confirmed complete on the VPS) but the ssh session carrying `./deploy.sh build`'s output hung after completion (path test showed healthy connectivity, not the 2026-09-02 packet-loss pattern; cause not identified); deploy-staging.sh read the killed ssh as a build failure and exited. Completed manually: `./deploy.sh staging` against the already-built image, then verify-apex.sh all 5 green | matthewkerns@Drs-MacBook-Pro |
| 2026-09-06T22:24Z | 9cfcc7e | Merge branch 'agent/cmo/launch-readiness' | lint/tsc/test green on VPS | staging-verified (script checks) | matthewkerns@Drs-MacBook-Pro |
...
| 2026-09-17T09:31Z | e0fa468 | docs: how to build this site from a fresh worktree | lint/tsc/test green on VPS | staging+apex-verified FROM THE VPS — the script's laptop-side verify read 000 on every route (Cox intercepts the domain on the home network; not a site failure, no rollback). VPS: apex / /about /privacy /terms /robots.txt /sitemap.xml all 200, www 200, staging smoke 8×200 + /subscribe 307, staging robots.txt disallow-all + x-robots-tag noindex,nofollow, both Traefik routers present, verify-apex.sh 5/5 incl. Safe Browsing clean | mango-prod-deploy (Matthew-authorized) |
