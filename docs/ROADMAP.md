...
**2026-09-08:** definitions decided by Matthew (communication = cadence + speed + quality[one-touch, closed-with-note, weekly self-rating, LLM judge off until reviewed]; delivery = progress[milestones, client-confirmed, hours on plan, shipped] × (0.5 + 0.5·time/10), time = due date / weekly movement / per-client SLA; 90 d; all focus tiles). Built on Mango `feat/client-scores` @ 9448fc2 (44 commits, pushed, not merged/deployed): 13-layer engine with a provisional gate, 06:30 refresh, tile + sparklines, 7 tools + API, single identity rule (tile key; refresh resolves TileIdentity). Gauntlet passed (three critics + 7am persona) including four read-only passes against the live data store that found and fixed seven attribution/None-vs-0 defects; ≥650 tests on the VPS lane. Definitions are recorded in the live config (chosen_by Matthew, 2026-09-08). Real numbers today: Trevor 7.6 / InfinityVault 3.8 communication; every other in-scope client has too little data to score (7 of 9) until due dates, a week plan, repo mappings and per-client Fathom/Slack attribution exist. Blind side-by-side on real renderings: win; 7am test: pass. Spec: `mango-income-tool/docs/client-scores/PRD.md` v10. Remaining: merge + deploy Mango; run `choose_score_definitions` (chosen_by Matthew); first real refresh; Q7 (self-rating scale) and Q3 (Lambda) confirmations.
· `priority: P1`
