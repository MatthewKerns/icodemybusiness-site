...
**2026-09-08:** definitions decided by Matthew (communication = cadence + speed + quality[one-touch, closed-with-note, weekly self-rating, LLM judge off until reviewed]; delivery = progress[milestones, client-confirmed, hours on plan, shipped] × (0.5 + 0.5·time/10), time = due date / weekly movement / per-client SLA; 90 d; all focus tiles). Built on Mango `feat/client-scores` @ 6916e86 (40 commits, pushed, not merged/deployed): 13-layer engine with a provisional gate, 06:30 refresh, tile + sparklines, 7 tools + API, single identity rule (tile key; refresh resolves TileIdentity). Gauntlet passed (three critics + 7am persona) including four read-only passes against the live data store that found and fixed seven attribution/None-vs-0 defects; ≥650 tests on the VPS lane. First provisional numbers exist (Trevor 7.6 / InfinityVault 3.8 communication); delivery is None for every client until due dates, a week plan and repo mappings exist. Spec: `mango-income-tool/docs/client-scores/PRD.md` v10. Remaining: merge + deploy Mango; run `choose_score_definitions` (chosen_by Matthew); first real refresh; Q7 (self-rating scale) and Q3 (Lambda) confirmations.
· `priority: P1`
